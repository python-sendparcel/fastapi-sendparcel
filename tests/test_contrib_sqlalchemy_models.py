...
async def test_shipment_model_has_timestamps_and_reference_id(
    async_session,
) -> None:
    """Verify ShipmentModel has created_at, updated_at, and reference_id."""
    mapper = inspect(ShipmentModel)
    column_names = {col.key for col in mapper.column_attrs}

    assert "created_at" in column_names
    assert "updated_at" in column_names
    assert "reference_id" in column_names


async def test_shipment_model_reference_id_indexed(async_session) -> None:
    """Verify reference_id column is indexed."""
    table = ShipmentModel.__table__
    indexed_columns = set()
    for idx in table.indexes:
        for col in idx.columns:
            indexed_columns.add(col.name)
    assert "reference_id" in indexed_columns
